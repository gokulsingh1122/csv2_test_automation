...
ENV Automation-Admin=Automation-Admin@competiscan.com
...
RUN mvn clean test -DsuiteXmlFile=suite-files/AdminSiteParent.xml -DtestEnvironment=uat -Dheadless=true -Dbrowser=firefox
CMD ["./start.sh"]
+